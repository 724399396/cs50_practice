...
char* indexes(const char* path)
{
    ...
    if (d)
    {
        ...
        {
            char* sub_name = strstr(dir->d_name, "index.php");
            if (sub_name != NULL) 
            {
            ...
            sub_name = strstr(dir->d_name, "index.html");
            if (sub_name != NULL) 
            {
...
const char* lookup(const char* path)
{
    char *extend = strrchr(path, '.');
    char *be_returned = malloc(16);
    if (extend == NULL) {
        return NULL;
    }
    if (strcasecmp(extend,".css") == 0)
    {
       strcpy(be_returned, "text/css");
    } else if(strcasecmp(extend,".html") == 0)
    {
        strcpy(be_returned,"text/html");
    } else if(strcasecmp(extend,".gif") == 0 )
    {
        strcpy(be_returned,"image/gif");
    } else if(strcasecmp(extend,".ico") == 0)
    {
        strcpy(be_returned,"image/x-icon");
    } else if(strcasecmp(extend,".jpg") == 0)
    {
        strcpy(be_returned,"image/jpeg");
    } else if(strcasecmp(extend,".js") == 0)
    {
        strcpy(be_returned,"text/javascript");
    } else if(strcasecmp(extend,".php") == 0)
    {
        strcpy(be_returned,"text/x-php");
    } else if(strcasecmp(extend,".png") == 0)
    {
        strcpy(be_returned,"image/png");
    } else {
        free(be_returned);
        be_returned = NULL;
    }
    if (be_returned != NULL) {
        char *res = malloc(strlen(be_returned) + 1);
        if (res == NULL) {
            return NULL;
        }
        strcpy(res, be_returned);
        free(be_returned);
        return res;
    }
    return NULL;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char *without_method = strchr(line,' ') + 1;
    char *without_request = strchr(without_method, ' ') + 1;
    int method_length = without_method - 1 - line;
    char *method = malloc(method_length + 1);
    strncpy(method, line, method_length);
    method[method_length] = '\0';
    int request_length = without_request - 1 - without_method;
    char *request_target = malloc(request_length + 1);
    strncpy(request_target, without_method, request_length);
    request_target[request_length] = '\0';
    int http_version_length = strlen(without_request) - 2;
    char *http_version = malloc(http_version_length + 1);
    strncpy(http_version, without_request, http_version_length);
    http_version[http_version_length] = '\0';
    if(strncmp("GET", method, method_length) != 0)
    {
        error(405);
        return false;
    }
    ...
    if(strncmp("HTTP/1.1", http_version, http_version_length) != 0)
    {
        error(505);
        return false;
    }
    char *parameter = strchr(request_target, '?');
    if (parameter == NULL)
    {
        strcpy(abs_path,  request_target);
        query[0] = '\0';
    } else {
        int parameter_length = parameter-request_target;
        strncpy(abs_path,  request_target, parameter_length);
        abs_path[parameter_length] = '\0';
        strcpy(query,  parameter + 1);
    }
    return true;
}
